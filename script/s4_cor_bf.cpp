...
int main(int argc,char *argv[])
{
	...
	j=0;
        infilename="temp_out/s3_"+samplestr+".txt";
		...
		while(p!=NULL)
		{
			...
			p = strtok(NULL,split);
                ...
                dis_ref[j-1]=myfunc.StringToDouble(str_temp[1]);
...
for(i=0;i<ident_num_ref;i++)
{
	...
	j=0;
	infilename="temp_out/s3_distri_"+samplestr+"_"+myfunc.IntToString(i+1)+".txt";
	infile.open(&infilename[0],ios::in);
	while(!infile.eof())
	{
		str_line="";
...
//		cout<<prob[i][j]<<endl;
	...
	outfilename="temp_out/s4_bf_"+samplestr+".txt";
