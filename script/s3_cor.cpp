...
int main(int argc,char *argv[])
{
	...
	vector<Markers> vec;
	int i,ii,beginmark=0,j=0,k=0,m=0,n,l,c,pop;
	ofstream outfile;
	ifstream infile;
	string str_line;
	...
	string gene_id[10000];
	string gene_name[10000];
	...
	j=0;
        ...
        while(!infile.eof())
        {
                ...
                gene_id[j-1]=str_temp[0];
                gene_name[j-1]=str_temp[1];
        }
        infile.close();infile.clear();
        gene_num=j;
	...
	for(i=0;i<ident_ref_num;i++)
	{
		...
		{
			if(ident_list_ref[i]==ident[j]){count++;index[i][count-1]=j;}
		}
	...
	j=0;
        infilename="data_obj/"+samplestr+".txt";
        infile.open(&infilename[0],ios::in);
        while(!infile.eof())
        {
                str_line="";
	...
	double ave_obj;
	double dis_cut;
	double sum1,sum2,sum3,mean1,mean2;
	double cor_sum[100000];
	double cor_obj[50];
	...
	{
			count_all=0;
			for(j=0;j<sampling_num;j++)
			{
				for(l=0;l<gene_num;l++)
                       		{
					exp_sum_obj[l]=0;
                              		exp_sum_ref[l]=0;
                       		}
				...
				for(k=0;k<cell_set;k++)
				{
					...
					rndnum2=1+rand()%cell_num_ref_each[i];
					for(l=0;l<gene_num;l++)
                       			{
						exp_sum_obj[l]+=*(exp_obj[rndnum1-1]+l);
                              			exp_sum_ref[l]+=*(exp_ref[index[i][rndnum2-1]]+l);
                       			}
				}
				for(l=0;l<gene_num;l++)
                       		{
				...
				//		if(match[l]=="0")continue;
                              			if(exp_ave_ref[l]==0||exp_ave_obj[l]==0)continue;
                              			count1++;
                              			sum1+=exp_ave_ref[l];
                              			sum2+=exp_ave_obj[l];
                       			}
					if(count1<100)continue;
					else
					{
						mean1=sum1/count1;
						mean2=sum2/count1;
				...
				//			if(match[l]=="0")continue;
                              				if(exp_ave_ref[l]==0||exp_ave_obj[l]==0)continue;
							sum3+=(exp_ave_ref[l]-mean1)*(exp_ave_obj[l]-mean2);
							sum1+=pow((exp_ave_ref[l]-mean1),2);
							sum2+=pow((exp_ave_obj[l]-mean2),2);
						}
						if(sum1==0||sum2==0)continue;	
						count_all++;
						cor_sum[count_all-1]=sum3/(pow(sum1,0.5)*pow(sum2,0.5));
					}
			}
			if(count_all==0){cor_obj[i]=0;}
			else 
			{
				sort(cor_sum,cor_sum+count_all);
				cor_obj[i]=cor_sum[count_all/2];
			}

		outfilename="temp_out/s3_distri_"+samplestr+"_"+myfunc.IntToString(i+1)+".txt";
                outfile.open(&outfilename[0],ios::out);
                for(j=0;j<count_all;j++)
                {
                        outfile<<cor_sum[j]<<endl;
                }
...
cout<<"cal correlation ... done."<<endl;

	outfilename="temp_out/s3_"+samplestr+".txt";
        outfile.open(&outfilename[0],ios::out);
                        for(j=0;j<ident_ref_num;j++)
                        {
                                outfile<<ident_list_ref[j]<<"\t"<<cor_obj[j]<<endl;
                        }
                        outfile.close();outfile.clear();
