...
bool comparer(Distance m_a, Distance m_b)//< asc, > desc
{
...
int main(int argc,char *argv[])
{
	...
	string ident[30000];
        string ident_list_ref[50];	

	int cell_num_ref;
	int cell_num_ref_each[5];
	int cell_num_obj;
	int rndnum1,rndnum2;
	int count1;
	double sum;
	...
	j=0;
		...
		k=0;
	        ...
	        cell_type[j-1]=str_temp[0];
        }
	infile.close();infile.clear();
        ident_ref_num=j;
	
	j=0;
        infilename="temp_out/s2_bf_"+samplestr+".txt";
        infile.open(&infilename[0],ios::in);
        while(!infile.eof())
        {
                getline(infile,str_line);
		...
		k=0;
	        ...
	        bf[0][j-1]=myfunc.StringToDouble(str_temp[1]);
        }
	infile.close();infile.clear();
//	cout<<"read in bf 1 "<<j<<endl;


	j=0;
        infilename="temp_out/s4_bf_"+samplestr+".txt";
        infile.open(&infilename[0],ios::in);
        while(!infile.eof())
        {
                getline(infile,str_line);
		...
		k=0;
	        ...
	        bf[1][j-1]=myfunc.StringToDouble(str_temp[1]);
        }
	infile.close();infile.clear();
//	cout<<"read in bf 2 "<<j<<endl;


	j=0;
        infilename="temp_out/s7_bf_"+samplestr+".txt";
        infile.open(&infilename[0],ios::in);
        while(!infile.eof())
        {
                getline(infile,str_line);
		...
		k=0;
	        ...
	        bf[2][j-1]=myfunc.StringToDouble(str_temp[1]);
        }
	infile.close();infile.clear();
//	cout<<"read in bf 3 "<<j<<endl;
	...
	for(j=0;j<ident_ref_num;j++)
	{
		...
		value1[1]=bf[1][j];
		value1[2]=bf[2][j];
		sort(value1,value1+3,comparer2);
		score[j]=pow(value1[0]*value1[1]*value1[2],0.33333);
	/*
		if(value1[0]>=0.6)score[j]=value1[0];
		else if(value1[0]<0.2)score[j]=pow(value1[0]*value1[1]*value1[2],0.33333);
		else if(value1[0]>=0.2&&value1[1]>=0.2&&value1[2]>=0.2)score[j]=pow(value1[0]*value1[1]*value1[2],0.33333);
		else if(value1[0]>=0.2&&value1[1]>=0.2&&value1[2]<0.2)score[j]=pow(value1[0]*value1[1],0.5);
		else if(value1[0]>=0.2&&value1[1]<0.2&&value1[2]<0.2)score[j]=value1[0];
		else score[j]=0;
	*/
		vec.push_back(Distance(score[j],cell_type[j]));
	}
	sort(vec.begin(), vec.end(), comparer);

	for(j=0;j<ident_ref_num;j++)
        {
        	close_dis[j]=vec[j].get_distance();
        	close_ident[j]=vec[j].get_ident();
        }	


	int index[50];
    for(j=0;j<ident_ref_num;j++)//sorted score
	{
		index[j]=-1;
       	for(k=0;k<ident_ref_num;k++)//original
		{
			if(close_ident[j]==cell_type[k])index[j]=k;
		}
	}

        cout<<"combine all Bayes Factors ... done."<<endl;

	outfilename="result/score_"+samplestr+".txt";
	outfile.open(&outfilename[0],ios::out);
    	outfile<<"cell_type\tBF1\tBF2\tBF3\tCCI"<<endl;
	for(j=0;j<ident_ref_num;j++)
	{
		outfile<<cell_type[index[j]]<<"\t"<<bf[0][index[j]]<<"\t"<<bf[1][index[j]]<<"\t"<<bf[2][index[j]]<<"\t"<<score[index[j]]<<endl;
	}
