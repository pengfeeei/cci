...
int main(int argc,char *argv[])
{
	...
	int sp_cell_num=20;//spl cell num
	int sp;
	vector<Distance> vec;
	...
	j=0;
        infilename="temp_out/s1_"+samplestr+".txt";
...
for(i=0;i<ident_num_ref;i++)
{
	...
	j=0;
	infilename="temp_out/s1_distri_"+samplestr+"_"+myfunc.IntToString(i+1)+".txt";
	infile.open(&infilename[0],ios::in);
	while(!infile.eof())
	{
		str_line="";
...
//		if(drop_ref[j]=="0")continue;
	...
	outfilename="temp_out/s2_bf_"+samplestr+".txt";
